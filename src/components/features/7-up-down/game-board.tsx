...
export const GameBoard: React.FC<PropsWithChildren<{ roundRecord: RoundRecord, amount: number, marketItems: RankedMarketItem[], roundRecordWithWinningId: RoundRecord | null }>> = ({ roundRecord, children, amount, marketItems, roundRecordWithWinningId }) => {
  ...
  const sortedStocks = marketItems.sort((a, b) => parseFloat(b.change_percent) - parseFloat(a.change_percent));
  ...
  return (
    <div style={{ backgroundImage: 'url(/images/7-up.png)', backgroundSize: 'cover', backgroundPosition: 'center' }} className="relative px-[6rem] pt-20 w-full pb-4 bg-brown-900 md:mx-auto">
      {children}
      <div className="flex justify-between ">
        ...
        <div className="grid grid-rows-7 w-[5rem] absolute right-0 h-full top-0">
          {stocksToDisplay.slice(7, 14).map((stock, index) => (
            <StockPrice key={index} rankedMarketItem={stock} />
          ))}
        </div>
