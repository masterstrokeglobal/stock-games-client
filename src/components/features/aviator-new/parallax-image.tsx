...
export default function ParallaxImage({ multiplier, isMoving = false }: ParallaxImageProps) {
  ...
  return (
    <div ref={imgContainerRef} className="parallax-image absolute inset-0 overflow-hidden  ">
      <div ref={imageRef} className="w-[calc(8000px*32)] h-[8000px] bg-lack  absolute inset-0 flex">
        {Array.from({ length: 32 }).map((_, i) => (
          <div
            key={i}
            className="w-[8000px] h-[8000px] absolute bg-[url('/images/aviator/aviator-bg.png')] bg-cover bg-no-repeat flip-alter-image"
