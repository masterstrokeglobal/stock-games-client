...
import {
  ...
  useGetMyCurrentRoundHeadTailPlacement,
} from "@/react-query/head-tail-queries";
...
const GameBoard = ({
  ...
  const { mutate: createHeadTailPlacement, isPending } =
    useCreateHeadTailPlacement();
  ...
  const handleCardClick = useCallback(
    (side: HeadTailPlacementType) => {
      if (isPending) return;
      createHeadTailPlacement({
        roundId: roundRecord.id,
        placement: side,
        amount: betAmount,
      });
    },
    [isPending, createHeadTailPlacement, roundRecord.id, betAmount]
  );

  const { data: placements } = useGetMyCurrentRoundHeadTailPlacement(
    roundRecord.id
  );

  // Memoize bet amounts calculation to prevent unnecessary re-renders
  ...
  // Memoize name calculations to prevent unnecessary re-renders
  const headName = useMemo(
    () =>
      roundRecord.market.find((m) => m.id === roundRecord.coinTossPair?.head.id)
        ?.codeName ?? "HEAD",
    [roundRecord.market, roundRecord.coinTossPair?.head.id]
  );

  const tailName = useMemo(
    () =>
      roundRecord.market.find((m) => m.id === roundRecord.coinTossPair?.tail.id)
        ?.codeName ?? "TAIL",
    [roundRecord.market, roundRecord.coinTossPair?.tail.id]
  );
  ...
  const winningStock = useMemo(() => {
    ...
    roundRecordWithWinningSide?.sortedMarketItems,
    stocks,
  ]);
    ...
    <section className={cn("flex flex-col relative w-full h-full", className)}>
      <main className="w-full h-full flex flex-col xxl:justify-around justify-between">
        <div className="xl:mx-20 -mx-4  relative xxl:pt-[15%] md:pt-20 pt-40">
          <img
            src="/images/head-tail/bg.png"
            alt="game board"
            className="w-full scale-125 -translate-y-1/4 z-0 h-full absolute top-0 left-0 object-cover"
          />
          <div className="bottom-0 left-0 w-full  min-h-40 bg-gradient-to-t scale-125 absolute z-0 from-[#00033D] to-transparent" />
          <img
            src="/images/head-tail/table.png"
            alt="table"
            className="w-full relative z-10 aspect-[8/3]"
            ref={tableRef}
          />
          <img
            src={
              !showLady
                ? "/images/head-tail/girl.gif"
                : "/images/head-tail/girl-to-flip.gif"
            }
            alt="lady"
            className={cn(
              "absolute z-0 xsm:left-1/2 left-[calc(48%)] -translate-x-1/2 xl:h-60 h-48 "
            )}
            style={{ bottom: tableHeight - 10 }}
          />
          ...
          <Suspense fallback={<div>Loading..</div>}>
            <CoinToss
              ...
              className={cn(
                "absolute z-20 left-1/2",
                showCoinToss ? "coin-toss-anim-1" : ""
              )}
              resultOutcome={roundRecordWithWinningSide?.winningSide}
            />
          </Suspense>
          <div
            style={{ bottom: tableHeight / 2 }}
            className="absolute w-full left-0 right-0 -translate-y-1/4 z-20 pointer-events-none"
          >
            <HeadCard
              winning={
                isPlaceOver && winningStock?.id === roundRecord.coinTossPair?.head?.id
              }
              ...
              name={headName}
              className={cn(
                "xs:left-[12%] left-[10px] -bottom-1/2 cursor-pointer",
            ...
            <TailCard
              winning={
                isPlaceOver && winningStock?.id === roundRecord.coinTossPair?.tail?.id
              }
              ...
              name={tailName}
              className={cn(
                "xs:right-[12%] right-[10px] -bottom-1/2 cursor-pointer",
        ...
        <div className="flex flex-col items-center">
          <GameTimer
            className="md:hidden flex relative z-10 mb-2"
            roundRecord={roundRecord}
          />
          <BettingArea
...
const HeadCard = ({
  ...
  <div
    ...
    style={{
      ...
      background:
        "radial-gradient(50% 50% at 50% 50%, rgba(57, 151, 201, 0.87) 0%, rgba(9, 39, 90, 0.87) 100%)",
  ...
  >
    ...
    {bet > 0 && (
      <BetIndicator
        key={bet}
        bet={bet}
        className="left-1/2 drop-shadow-none -translate-x-1/2 top-0 -translate-y-full"
      />
    )}
    ...
    >
      <span
        className={cn(
          "font-medium tracking-wider text-[#6CDCFB] drop-shadow font-protest-strike",
    ...
    <div className="flex flex-col items-center flex-1 justify-center relative w-full">
      <span
        className={cn(
          "tracking-wider text-white  text-center mb-0.5 font-prosto-one w-full px-1 line-clamp-1 truncate",
          isMobile ? "text-xs" : "text-sm"
        )}
      >
        {name.toUpperCase()}
      ...
      <div className="flex items-center justify-center w-full my-1">
        <img
          ...
          alt="head coin"
          className={cn(
            "relative z-10 object-contain drop-shadow",
            isMobile
              ? "w-8 h-8 scale-100 -translate-y-0.5"
              : "w-14 -translate-y-0.5 h-14"
          )}
          style={{
            maxWidth: isMobile ? 32 : 60,
            maxHeight: isMobile ? 32 : 60,
          }}
        />
        <div
          className={cn(
            "w-auto aspect-square blur-sm rounded-full absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#001D4B]",
            isMobile ? "h-8" : "h-[60px]"
          )}
        ></div>
      </div>
      <div
        className={cn(
          "font-bold text-white font-inter mb-0.5",
    ...
    {/* Bottom Stats */}
    <div className="w-full px-2 py-1.5 flex flex-col rounded-b-xl overflow-hidden ">
      ...
      <div className="flex flex-row justify-between w-full">
        <span className="text-[10px] font-phudu font-normal text-white">
          {INR(bet)}
        </span>
        <span className="text-[10px] font-phudu font-normal text-white">
          {INR(win)}
...
const TailCard = ({
  ...
  >
    ...
    >
      <span className="text-xs md:text-sm font-medium tracking-wider text-[#6CDCFB] drop-shadow font-protest-strike">
        TAIL
      </span>
    </div>
    {/* SubLabel and Coin */}
    <div className="flex flex-col items-center flex-1 justify-center relative w-full">
      <span
        className={cn(
          "tracking-wider text-white text-center mb-0.5 w-full px-1 line-clamp-1 truncate font-prosto-one",
          isMobile ? "text-xs" : "text-sm"
        )}
      >
        {name.toUpperCase()}
      ...
      <div className="flex items-center justify-center w-full my-1">
        <img
          ...
          alt="tail coin"
          className={cn(
            "relative z-10 object-contain drop-shadow -translate-y-0.5",
            isMobile ? "w-8 h-8 scale-100" : "w-14 -translate-y-0.5 h-14"
          )}
          style={{
            maxWidth: isMobile ? 32 : 60,
            maxHeight: isMobile ? 32 : 60,
...
const BetIndicator = ({
  ...
  return (
    <div
      className={cn(
        "pointer-events-auto rounded-lg absolute flex flex-col items-center z-20 ",
        className
      )}
    >
      <div
      ...
      >
        ...
        <div className="flex flex-col items-start">
          <span
            className="text-white leading-none font-semibold font-poppins flex items-center text-xs md:text-sm"
            style={{ textShadow: "0px 0px 2px #24FA13" }}
          >
            {INR(bet, true)}
          </span>
          <span
            className="text-white font-poppins text-[10px] md:text-xs"
            style={{ marginTop: "-1px", textShadow: "0px 0px 2px #24FA13" }}
          >
            placed
          </span>
        </div>
