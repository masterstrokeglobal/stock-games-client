        ...
        <Popover open={open} onOpenChange={setOpen} >
            ...
            <PopoverContent className="md:w-80 w-64 p-0 border-none bg-transparent" align="end" onInteractOutside={(e) => e.preventDefault()} onPointerDownOutside={(e) => e.preventDefault()}     >
                <div style={{
                    ...
                    {/* Header */}
                    <div className="flex items-center border-b border-[#0074FF]  bg-[#004DA9] justify-between px-6 py-3">
                        <div className="flex items-center space-x-3">
                            <Avatar>
                                <AvatarImage src={user?.profileImage} />
                                <AvatarFallback>{user?.name?.charAt(0)}</AvatarFallback>
                            </Avatar>
                            <span className="text-white  font-medium font-inter uppercase">{user?.name}</span>
                        </div>

                        <PopoverClose asChild>
                            <button
                                className="text-white hover:text-gray-200 text-xl px-2 font-play transition-colors"
                            >
                                X
                            </button>
                        </PopoverClose>
                    ...
                    <div className="p-6 pt-12">
                        <Link href="/game/platform">
                            <button className="w-full bg-[#004DA9] hover:bg-[#0074FF] text-white text-lg font-medium py-2 rounded-2xl transition-colors shadow-lg">
                                Home
                            </button>
                        </Link>
                    </div>
