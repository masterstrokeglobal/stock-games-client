...
const WheelOfFortune: React.FC<WheelOfFortuneProps> = ({
  ...
  useEffect(() => {
    if (wheelRef.current) {
      setWheelHeight(wheelRef.current.clientHeight);
    }
  }, [wheelRef.current]);

  const winningColor = useMemo(() => {
    if (!winningMarketId || !roundRecord) return null;
    const color = roundRecord.getColorByMarketId(
      winningMarketId[0] as unknown as number
    );
  ...
  return (
    <div
      className={`flex flex-col items-center w-full relative gap-6 ${
        className || ""
      }`}
    >
      <div
      ...
      {showResult &&
        ...
        winningMarketId.length > 0 && (
          <>
            <div
              style={{
                backgroundImage: winningColor
                  ? winningColor.backgroundGradient
                  : "transparent",
                boxShadow: winningColor ? winningColor.shadow : "none",
                borderColor: winningColor
                  ? winningColor.borderColor
                  : "transparent",
              }}
              className={cn(
                "w-full backdrop-blur-lg  md:max-w-md xs:max-w-screen-xs max-w-72  rounded-md z-40 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 p-4 border-2"
              )}
            >
              <div className="text-center font-medium uppercase">
                ...
                <hr
                  ...
                  className={cn(" h-0.5 border-none my-4 mx-[25%]")}
                />
