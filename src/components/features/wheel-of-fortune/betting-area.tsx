...
export const BettingArea: React.FC<BettingAreaProps> = ({
  ...
  const totalBetAmount = useMemo(() => {
    return placements?.reduce((acc, curr) => acc + curr.amount, 0) || 0;
  }, [placements]);
  ...
  return (
    <>
      <div
        className={cn(
          "w-full flex flex-col font-montserrat tracking-wide justify-between lg:gap-4 gap-2 text-white ",
          className
        )}
      >
        <div className="grid grid-cols-4 lg:grid-cols-2 gap-2 w-full">
          {coinValues?.map((amount) => (
            <Button
              className={cn(
                "rounded-md transition-all py-2 lg:py-4 h-8 lg:h-12 duration-200 text-white relative group overflow-hidden bg-[#366D51] border-2 border-[#5DA69A] hover:bg-[#366D51] hover:border-[#5DA69A] "
              )}
              key={amount}
        ...
        <div className="flex justify-center relative lg:mb-4">
          <Button
            ...
            className="rounded-full absolute left-2 top-2  h-8 w-10 px-0 flex items-center justify-center bg-[#366D51] border border-[#5DA69A]"
          >
          ...
          <Button
            ...
            className="rounded-full absolute right-2 top-2  h-8 w-10 px-0 flex items-center justify-center bg-[#366D51] border border-[#5DA69A]"
          >
        ...
        >
          ...
          <span className="font-bold">{INR(totalBetAmount, true)}</span>
        </div>
      </div>
      <WheelOfFortuneResultDialog
        key={String(showResult.showResults)}
        open={showResult.showResults}
        roundRecordId={showResult.previousRoundId ?? 0}
      />
    </>
  );
};
