...
export const BettingArea: React.FC<BettingAreaProps> = ({
    ...
    const { currentStocks, stockPrice } = useMemo(() => {
        const currentStocks: MarketItem[] = roundRecord.market.sort((a, b) => a.name?.localeCompare(b.name ?? '') ?? 0);
        ...
        else if (winningIdRoundRecord?.finalPricesPresent && winningIdRoundRecord?.finalDifferences) {
                ...
                <div className="flex items-center justify-center gap-[100px]">
                    ...
                    <div className="relative w-24 h-24 group ">
                        ...
                        <button
                            ...
                            <div
                                style={{ animationDuration: "5s" }} className="rounded-full border-4 animate-spin border-[#E3B872] bg-[#2A1810]/80 flex items-center justify-center transition-all duration-300 group-hover:border-[#FFD700] group-hover:shadow-[0_0_20px_rgba(255,215,0,0.5)] group-hover:bg-[#2A1810]">
                                <Image
