...
const StockCard = ({ stock, className, amount, roundRecord }: { stock?: RankedMarketItem, className?: string, amount?: number, roundRecord?: RoundRecord }) => {
    ...
    return (
        <div className="relative  bg-[#D9D9D9] rounded-xl p-1">
            <div className={cn(
                "w-10 md:w-16 lg:w-20 min-h-14 md:min-h-16 lg:min-h-20 rounded-lg shadow-lg relative transform  transition-transform cursor-pointer md:p-2 p-1",
                className
            )} style={{ boxShadow: "0px 0px 1.5px 1px #005A7B" }} >
                <div className="h-full flex flex-col items-center justify-center sm:gap-0.5">
                    <span className="font-spacemono text-[8px] md:text-[10px] lg:text-xs text-[#005C80] text-center font-bold truncate w-full">{stock.name}</span>


                    <span className={cn(" whitespace-nowrap  font-space-grotesk text-center md:text-xs text-[8px]  font-bold truncate w-full", Number(stock.change_percent) >= 0 ? "text-[#519900]" : "text-[#E60004]")}>
                        {stock.currency}
                    ...
                    <div className=" bg-[#005A7B] border-[#7DB7BB] border-2 w-full text-white font-orbitron md:text-[8px] text-[6px]  text-center mx-auto px-2 py-0.5 rounded-full font-bold shadow-lg">
                        {amount ? `₹${amount}` : "-"}
                    </div>
                </div>
            </div>
