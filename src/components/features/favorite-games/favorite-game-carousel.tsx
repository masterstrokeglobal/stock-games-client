...
export default function FavoriteGameCarousel({providerCompany,title}: {providerCompany?: ProviderCompany,title?: string}) {
    ...
    const filteredFavorites = providerCompany ? favorites.filter((favorite) => favorite.game?.providerCompany === providerCompany) : favorites;
    return (
        <Carousel opts={{ loop: false, startIndex: 0 }} className="w-full pt-0.5">
            <div className="space-y-1">
                <div className="flex items-center justify-between">
                    <h2 className="md:text-2xl sm:text-base text-sm font-semibold text-platform-text">{title || "Favorite Games"}</h2>
                        ...
                        <CarouselItem key={index} className="xs:basis-1/3 basis-1/2 md:basis-1/4 lg:basis-1/5 xl:basis-1/6 pl-4">
                            {
                                game.gameType ? (
                                    ...
                                    <GameCard className={providerCompany === ProviderCompany.GAP ? "aspect-[5/3]" : ""} game={game.game!} />
                                )
                            }                        </CarouselItem>
                    ))}
                </CarouselContent>
