...
import { GameCategories, GameCategory } from "@/models/casino-games"
...
export default function GamingAppInterface() {
    ...
    const areFiltersApplied = filter.search || filter.category !== "all";
    return (
        <>
            <GameAdsCarousel />
            <main className="container mx-auto mt-20 px-4 py-6">
                {/* Search Bar */}
                <div className="relative mb-8  mx-auto flex justify-start gap-2">
                    ...
                    <Select onValueChange={(value) => setFilter({ ...filter, category: value })} value={filter.category}>
                        <SelectTrigger className="w-full max-w-40 bg-background-secondary focus:bg-background-secondary/80 border-transparent border-2 focus:border-game-secondary text-white placeholder:text-gray-400  h-12 rounded-full">
                            <SelectValue placeholder="Select Category" />
                        </SelectTrigger>
                        <SelectContent className="bg-background-secondary rounded-xl border-game-secondary">
                            <SelectItem value="all" className="text-white">All</SelectItem>
                            {GameCategories.map((category) => (
                                <SelectItem key={category.value} value={category.value} className="text-white">{category.label}</SelectItem>
                            ))}
                        </SelectContent>
                    </Select>

                        {/* platforms  */}
                        <Select onValueChange={(value) => setFilter({ ...filter, platform: value })} value={filter.platform}>
                            <SelectTrigger className="w-full max-w-40 bg-background-secondary focus:bg-background-secondary/80 border-transparent border-2 focus:border-game-secondary text-white placeholder:text-gray-400  h-12 rounded-full">
